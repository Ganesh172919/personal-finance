...
export const authAny: RequestHandler = (req, res, next) => {
  passport.authenticate("jwt", { session: false }, async (err: unknown, user: unknown) => {
    try {
      ...
      if (!rawKey) {
        res.status(401).json({ message: "Unauthorized", code: "UNAUTHORIZED", request_id: req.requestId });
        return;
      }

      const key = await resolveApiKey(rawKey);
      if (!key) {
        res.status(401).json({ message: "Invalid API key", code: "API_KEY_INVALID", request_id: req.requestId });
        return;
      }
      ...
      if (mongoose.Types.ObjectId.isValid(createdByUserId) && mongoose.Types.ObjectId.isValid(orgId)) {
